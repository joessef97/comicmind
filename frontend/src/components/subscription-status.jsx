    ...
    return (
      <section className="mt-4 rounded-xl border border-green-300/60 bg-green-50 p-5 shadow-sm transition-all hover:shadow-md dark:border-green-500/30 dark:bg-green-950/20">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h3 className="text-lg font-semibold text-green-800 dark:text-green-300">Current Plan</h3>
          <Button
            type="button"
            variant="outline"
            size="sm"
            className="border-green-700/30 bg-white/60 text-green-900 hover:bg-white dark:bg-green-950/20 dark:text-green-100"
            onClick={openPortal}
        ...
        <div className="mt-3 grid gap-2 text-sm text-green-900 dark:text-green-200 md:grid-cols-2">
          <p><span className="font-medium">Plan:</span> {data.currentPlan || data.packageName || "-"}</p>
          <p><span className="font-medium">Status:</span> {data.status || "inactive"}</p>
        </div>
        <div className="mt-4">
          <div className="mb-2 flex items-center justify-between text-xs text-green-800 dark:text-green-300">
            <span>Comics used this month</span>
            <span>{data.comicsUsed} / {data.comicsLimit} comics used</span>
          </div>

...
import type { ImageProvider, ImageGenerationMeta } from "./services/image-provider";
import { getOpenAIImageService } from "./services/openai-image-service";
...
for an AI image generator (DALL-E 3).
...
CRITICAL CONSTRAINT: DALL-E 3 has ZERO MEMORY between panels. The ONLY way
...
export async function generateCharacterReference(
  ...
  storyIdea: string,
  style: string,
): Promise<{ imageBuffer?: Buffer; imageUrl: string; meta: import("./services/image-provider").ImageGenerationMeta }> {

...
export async function createComic(req: AuthRequest, res: Response) {
  try {
    ...
    if (comicsUsed >= comicsLimit) {
      return res.status(403).json({ message: "Comic creation limit reached for your subscription" });
    }
    ...
    await UserModel.updateOne(
      { _id: req.userId, "subscription.isActive": true },
      { $inc: { "subscription.comicsUsed": 1 } },
    );
